...
array=($(ls run-exp3-c-*.json))
mpirun -np  48 ./benchmark_01 json "${array[@]}" | tee exp3b_annulus.txt

array=($(ls run-exp3-d-*.json))
mpirun -np  48 ./benchmark_01 json "${array[@]}" | tee exp3b_quadrant.txt
